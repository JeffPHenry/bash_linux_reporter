...
nvidia-smi
date
#check if vpn is connected as tun0 interface
vpn_address=`/sbin/ifconfig tun0 | grep inet | cut -d' ' -f12-13 | sed 's/addr://g'`
if [ ! -n "$vpn_address" ]
then
                echo "VPN IS DOWN ON MACHINE!"
else
		echo "VPN IS UP AND RUNNING ON MACHINE AT: $vpn_address"
fi
